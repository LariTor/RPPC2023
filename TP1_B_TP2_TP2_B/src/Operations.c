...
/*BREVE DESCRIPCIÓN: En este archivo se implementan las funciones:
    ...
    -> RREF -> Hace la matriz de la forma Reduced Row Echelon Form.
    -> row ->  Returnea el ith row de la matriz
    -> Add -> sumo 2 matrices, SI ES POSIBLE, sino error
    -> Exchange ->  Cmabiar las rows i and j en la matriz
    -> Multiply ->  Multiplica dos matrices si es posible, sino tira error.
    -> Scale -> Escala una matriz por lambda
    -> ScaleRow -> Escala la fila por lambda
    -> AddRow -> Agrega una fila r en el ith row de la matrix
*/

#include "Operations.h"

Value Determinant(Matrix *mat){
    //void assert(int expression) allows diagnostic information to be written to the standard error file
...
Matrix Add (Matrix *mat1, Matrix *mat2) {
  // Este asser funciona para tirar error si las matrices no tienen igual dimension
...
Matrix Multiply (Matrix *mat1, Matrix *mat2) {
  //Se fija si es posible realizar la multiplicacion, sino error
...
void Exchange (Matrix *mat, Index ith, Index jth) {
  // Chequea si el indice i y j estan en rango de la matrz
